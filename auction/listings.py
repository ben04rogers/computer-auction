...
def showlisting(id):
    ...
    return render_template(
        "listings/showlisting.html",
        listing=listing,
        form=review_form_instance,
        bidform=bid_form_instance,
    )
...
def createlisting():
    ...
    if request.method == "POST":
        if not listingform.validate():
            for field, errors in listingform.errors.items():
                for error in errors:
                    flash(f"{field}: {error}", "danger")
            # Try to parse date manually if it failed
            if "end_date" in request.form and request.form["end_date"]:
                from datetime import datetime

                try:
                    listingform.end_date.data = datetime.strptime(
...
def close_listing(listing):
    ...
    if update_listing.total_bids > 0:
        ...
        highest_bid = Bid.query.filter_by(
            listing_id=listing, bid_status="Winning"
        ).first()
...
def remove_watchlist(listing):
    update_watchlist = WatchListItem.query.filter_by(
        listing_id=listing, user_id=current_user.id
    ).first()
...
def review(listing):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("listing.showlisting", id=listing) + "?tab=reviews")
